            ...
            <div className='contents' key={rec?.[0]?._id} >
                {rec?.map((item)=>(
                    <ProductCard  
                        ...
                        <h1 className='absolute top-0 left-0 text-xs text-left px-3 mt-3 font-bold text-white bg-green-500 py-1 rounded-r-md'>Best Sellers in <span className='text-primary'>{item.category}</span></h1>
                    </ProductCard>
                ))}
            </div>
        )): (
